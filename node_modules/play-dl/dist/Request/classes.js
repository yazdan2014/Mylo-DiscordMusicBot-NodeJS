"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Response = void 0;
const tls_1 = __importDefault(require("tls"));
const url_1 = require("url");
const LiveStream_1 = require("../YouTube/classes/LiveStream");
class Response {
    constructor(req_url, options) {
        this.parsed_url = new url_1.URL(req_url);
        this.sentHeaders = '';
        this.statusCode = 0;
        this.sentBody = '';
        this.rawHeaders = '';
        this.body = '';
        this.headers = {};
        this.timer = null;
        this.options = options;
        this.socket = tls_1.default.connect({
            host: this.parsed_url.hostname,
            port: Number(this.parsed_url.port) || 443,
            socket: options.socket,
            rejectUnauthorized: false
        }, () => this.onConnect());
        if (options.headers) {
            for (const [key, value] of Object.entries(options.headers)) {
                this.sentHeaders += `${key}: ${value}\r\n`;
            }
        }
        if (options.body)
            this.sentBody = options.body;
    }
    onConnect() {
        this.socket.write(`${this.options.method} ${this.parsed_url.pathname}${this.parsed_url.search} HTTP/1.1\r\n` +
            `Host : ${this.parsed_url.hostname}\r\n` +
            this.sentHeaders +
            `Connection: close\r\n` +
            `\r\n` +
            this.sentBody);
    }
    parseHeaders() {
        var _a, _b;
        const head_arr = this.rawHeaders.split('\r\n');
        this.statusCode = (_b = Number((_a = head_arr.shift()) === null || _a === void 0 ? void 0 : _a.split(' ')[1])) !== null && _b !== void 0 ? _b : -1;
        for (const head of head_arr) {
            let [key, value] = head.split(': ');
            if (!value)
                break;
            key = key.trim().toLowerCase();
            value = value.trim();
            if (Object.keys(this.headers).includes(key)) {
                let val = this.headers[key];
                if (typeof val === 'string')
                    val = [val];
                Object.assign(this.headers, { [key]: [...val, value] });
            }
            else
                Object.assign(this.headers, { [key]: value });
        }
    }
    stream() {
        return new Promise((resolve, reject) => {
            this.timer = new LiveStream_1.Timer(() => this.socket.end(), this.options.timeout || 1);
            this.socket.once('error', (err) => reject(err));
            this.socket.once('data', (chunk) => {
                this.rawHeaders = chunk.toString('utf-8');
                this.parseHeaders();
                resolve(this.socket);
            });
            this.socket.on('data', () => { var _a; return (_a = this.timer) === null || _a === void 0 ? void 0 : _a.reuse(); });
            this.socket.once('end', () => { var _a; return (_a = this.timer) === null || _a === void 0 ? void 0 : _a.destroy(); });
        });
    }
    fetch() {
        return new Promise((resolve, reject) => {
            this.socket.setEncoding('utf-8');
            this.socket.once('error', (err) => reject(err));
            this.socket.on('data', (chunk) => {
                if (this.rawHeaders.length === 0) {
                    this.rawHeaders = chunk;
                    this.parseHeaders();
                }
                else {
                    const arr = chunk.split('\r\n');
                    if (arr.length > 1 && arr[0].length < 5)
                        arr.shift();
                    this.body += arr.join('');
                }
            });
            this.socket.on('end', () => {
                resolve(this);
            });
        });
    }
}
exports.Response = Response;
//# sourceMappingURL=classes.js.map